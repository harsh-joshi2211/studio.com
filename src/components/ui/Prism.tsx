...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const render = (t: number) => {
      ...
      if (animationType === 'hover') {
        ...
        if (NOISE_IS_ZERO && Math.abs(yaw - targetYaw) < 1e-4 && Math.abs(pitch - targetPitch) < 1e-4) {
          continueRAF = false;
        }
      } else if (animationType === '3drotate') {
    ...
    // @ts-ignore - IO custom property tracking
  ...
  }, [height, baseWidth, animationType, glow, offset?.x, offset?.y, scale, transparent, hueShift, colorFrequency, timeScale, hoverStrength, inertia, bloom, suspendWhenOffscreen]);
